...
def extract_A(pixels,pk1,z1,pk2,z2,capacity,origimg,locationmap):
    ...
    fluctuatedValue = IDEA_Fluctuated_Value.black_calculate_fluctuated_value(pixels)
    ...
    oriB = tool.recoverflow(origB,locationmap)

    cv.imwrite(origimg, oriB,[cv.IMWRITE_PNG_COMPRESSION, 0])
...
def extract_B(image,pk1,z1,pk2,z2,capacity):
    ...
    fluctuatedValue = IDEA_Fluctuated_Value.white_calculate_fluctuated_value(pixels)
    ...
    recgpixB = np.array(pixels)
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            if (i + j) % 2 != 0:  # ·集 黑色区域
                recgpixB[i][j] = predictValue[i][j] + recoverpe2x2[i][j]

    return secret_message,recgpixB
